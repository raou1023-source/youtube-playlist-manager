...
class PlaylistManagerGUI:
    ...
    # タブ別キーワード定義
    KEYWORD_TABS = {
        "音楽": ["rock", "jazz", "pop", "classical", "hip-hop", "electronic", "country", "reggae", "blues", "metal"],
        "映画": ["action", "comedy", "drama", "horror", "sci-fi", "animation", "documentary", "thriller"],
        "教育": ["science", "history", "math", "language", "technology", "tutorial", "lecture"],
        "ニュース": ["world news", "politics", "economy", "sports news", "technology news"],
    }

    # 地域タブ - 「全世界」は「その他」タブの先頭に配置
    REGION_TABS = {
    ...
    def _create_menu(self):
        ...
        file_menu.add_command(label="履歴をエクスポート", command=self._export_history)
        file_menu.add_command(label="履歴をインポート", command=self._import_history)
        ...
        settings_menu.add_command(label="セットアップウィザード", command=self._open_setup_wizard)
        settings_menu.add_separator()
        settings_menu.add_command(label="YouTube認証", command=self._reset_credentials)
        settings_menu.add_command(label="ニコニコ動画認証", command=self._open_niconico_auth_dialog)
        settings_menu.add_separator()
        settings_menu.add_command(label="認証状態を確認", command=self._check_auth_status)

        # ヘルプメニュー
        help_menu = tk.Menu(self.menubar, tearoff=0)
        self.menubar.add_cascade(label="ヘルプ", menu=help_menu)
        help_menu.add_command(label="YouTube API取得方法", command=self._show_youtube_api_help)
        help_menu.add_command(label="ニコニコ動画について", command=self._show_niconico_help)
        help_menu.add_separator()
        help_menu.add_command(label="使い方", command=lambda: self._open_url_safely("https://developers.google.com/youtube/v3/getting-started"))
        help_menu.add_command(label="トラブルシューティング", command=lambda: self._open_url_safely("https://developers.google.com/youtube/v3/getting-started#before-you-start"))
        help_menu.add_separator()
        help_menu.add_command(label="更新を確認", command=self._check_for_updates)
        help_menu.add_separator()
...
- 一部のAPIレスポンスが制限される場合あり""",
    ...
    def _create_widgets(self):
        ...
        # 年代
        ttk.Label(basic_grid, text="年代:").grid(row=0, column=0, sticky=tk.W, pady=5, padx=(0, 10))
        ...
        # カテゴリ
        ttk.Label(basic_grid, text="カテゴリ:").grid(row=0, column=2, sticky=tk.W, pady=5, padx=(0, 10))
        ...
        ttk.Label(count_frame, text="動画数:").pack(side=tk.LEFT, padx=(0, 10))
        ...
        ttk.Label(count_frame, text="(1-50)", foreground="gray").pack(side=tk.LEFT, padx=(5, 0))
        ...
        keyword_frame.pack(fill=tk.X, pady=(0, 20))
        ...
        for tab_name, keywords in self.KEYWORD_TABS.items():
            tab_frame = ttk.Frame(self.keyword_notebook)
            self.keyword_notebook.add(tab_frame, text=tab_name)
        ...
        self.keyword_notebook.add(region_tab_frame, text="地域")
        ...
        all_regions = []
        for regions in self.REGION_TABS.values():
        ...
        # 追加キーワード入力欄
        additional_frame = ttk.Frame(keyword_frame)
        additional_frame.pack(fill=tk.X, pady=(15, 0))
        ttk.Label(additional_frame, text="追加キーワード:").pack(side=tk.LEFT)
        ...
        # 選択中キーワード表示（大きく）
        selected_frame = ttk.Frame(keyword_frame)
        selected_frame.pack(fill=tk.X, pady=(10, 0))
        self.selected_keywords_var = tk.StringVar(value="選択中: なし")
        ...
        # 選択中の地域表示
        selected_regions_frame = ttk.Frame(keyword_frame)
        selected_regions_frame.pack(fill=tk.X, pady=(10, 0))
        self.selected_regions_var = tk.StringVar(value="選択中: 全世界")
        ...
        # 地域オプション
        region_options_frame = ttk.Frame(keyword_frame)
        region_options_frame.pack(fill=tk.X, pady=(5, 0))

        self.add_region_keywords_var = tk.BooleanVar(value=True)
        region_keyword_cb = ttk.Checkbutton(
            region_options_frame,
            text="地域キーワードを自動追加",
            variable=self.add_region_keywords_var
        )
        region_keyword_cb.pack(side=tk.LEFT)

        # 地域コード表示ラベル
        self.region_code_var = tk.StringVar(value="(regionCode: なし)")
        ...
        # セクション3: 検索オプション（拡張版）
        # ========================================
        search_options_frame = ttk.LabelFrame(
            self.scrollable_frame,
            text="検索オプション",
            padding="15",
            style="Section.TLabelframe"
        )
        search_options_frame.pack(fill=tk.X, pady=(0, 20))

        # 検索精度セクション
        precision_label = ttk.Label(search_options_frame, text="検索精度:", font=("", 10, "bold"))
        precision_label.pack(anchor=tk.W)

        self.search_precision_var = tk.StringVar(value="standard")

        # 各精度オプションを詳細説明付きで表示
        precision_options = [
            ("standard", "標準（デフォルト）", "通常の検索 + 公式チャンネル優先"),
            ("high", "高精度", "登録済み公式チャンネルのみ"),
            ("highest", "最高精度", "チャンネルIDリストから直接検索"),
        ]

        for value, label, description in precision_options:
            option_frame = ttk.Frame(search_options_frame)
        ...
        # セパレータ
        ttk.Separator(search_options_frame, orient="horizontal").pack(fill=tk.X, pady=15)

        # 公式チャンネル優先設定
        official_label = ttk.Label(search_options_frame, text="公式チャンネル優先設定:", font=("", 10, "bold"))
        official_label.pack(anchor=tk.W)

        official_options_frame = ttk.Frame(search_options_frame)
        official_options_frame.pack(fill=tk.X, padx=(20, 0), pady=(5, 0))

        self.prefer_official_var = tk.BooleanVar(value=True)
        prefer_official_cb = ttk.Checkbutton(
            official_options_frame,
            text="公式チャンネル優先",
            variable=self.prefer_official_var
        )
        prefer_official_cb.grid(row=0, column=0, sticky=tk.W, pady=2)

        # 追加の詳細設定
        self.require_verified_var = tk.BooleanVar(value=False)
        verified_cb = ttk.Checkbutton(
            official_options_frame,
            text="認証済みバッジ必須",
            variable=self.require_verified_var
        )
        verified_cb.grid(row=0, column=1, sticky=tk.W, padx=(30, 0), pady=2)

        self.min_subscribers_var = tk.BooleanVar(value=False)
        subscribers_cb = ttk.Checkbutton(
            official_options_frame,
            text="チャンネル登録者数: 100万人以上",
            variable=self.min_subscribers_var
        )
        subscribers_cb.grid(row=1, column=0, sticky=tk.W, pady=2)

        self.min_views_var = tk.BooleanVar(value=False)
        views_cb = ttk.Checkbutton(
            official_options_frame,
            text="視聴回数: 100万回以上",
            variable=self.min_views_var
        )
        views_cb.grid(row=1, column=1, sticky=tk.W, padx=(30, 0), pady=2)

        self.vevo_only_var = tk.BooleanVar(value=False)
        vevo_cb = ttk.Checkbutton(
            official_options_frame,
            text="VEVO/公式のみ",
            variable=self.vevo_only_var
        )
        vevo_cb.grid(row=2, column=0, sticky=tk.W, pady=2)

        # 詳細説明オプション
        self.add_detailed_description_var = tk.BooleanVar(value=True)
        detailed_desc_cb = ttk.Checkbutton(
            official_options_frame,
            text="詳細な説明を追加",
            variable=self.add_detailed_description_var
        )
        detailed_desc_cb.grid(row=2, column=1, sticky=tk.W, padx=(30, 0), pady=2)
        ...
        privacy_options_frame = ttk.Frame(privacy_frame)
        ...
        privacy_options = [
            ...
            ("public", "公開", "誰でも検索・閲覧可能"),
        ]

        for i, (value, label, desc) in enumerate(privacy_options):
            rb = ttk.Radiobutton(
        ...
        platform_options_frame = ttk.Frame(platform_frame)
        ...
        ttk.Label(preset_row, text="プリセット:").pack(side=tk.LEFT)
        ...
        for text, cmd in [("読込", self._load_preset), ("保存", self._save_preset),
                          ("削除", self._delete_preset), ("編集", self._edit_preset)]:
            ttk.Button(preset_row, text=text, command=cmd, width=8).pack(side=tk.LEFT, padx=(0, 5))
        ...
        progress_label_frame = ttk.LabelFrame(footer_frame, text="進行状況", padding="8")
        progress_label_frame.pack(fill=tk.X)

        self.progress_var = tk.StringVar(value="待機中...")
        self.progress_label = ttk.Label(progress_label_frame, textvariable=self.progress_var)
        ...
        # 結果表示用のスクロール可能なフレーム
        result_canvas = tk.Canvas(result_frame, height=150, highlightthickness=0)
        ...
        ttk.Label(url_frame, text="再生リストURL:").pack(side=tk.LEFT)
        ...
        self.copy_btn = ttk.Button(url_frame, text="コピー", command=self._copy_url, width=8, state=tk.DISABLED)
        self.copy_btn.pack(side=tk.LEFT, padx=(5, 0))

        self.open_btn = ttk.Button(url_frame, text="開く", command=self._open_playlist, width=8, state=tk.DISABLED)
        ...
        for text, cmd, width in [("更新", self._refresh_history, 8), ("全削除", self._clear_history, 8),
                                  ("エクスポート", self._export_history, 12), ("インポート", self._import_history, 12)]:
            ttk.Button(history_btn_frame, text=text, command=cmd, width=width).pack(side=tk.LEFT, padx=(0, 5))
        ...
        self.history_tree.heading("date", text="作成日時")
        self.history_tree.heading("title", text="タイトル")
        self.history_tree.heading("videos", text="動画数")
        self.history_tree.heading("platform", text="Platform")
        self.history_tree.heading("category", text="カテゴリ")
        self.history_tree.heading("era", text="年代")
        ...
        for text, cmd, width in [("同条件で再作成", self._recreate_from_history, 14),
                                  ("URLを開く", self._open_history_url, 12),
                                  ("📺 動画確認", self._view_history_videos, 12),
                                  ("履歴を削除", self._delete_history_entry, 12),
                                  ("CSV出力", self._export_history_csv, 10)]:
            ttk.Button(history_action_frame, text=text, command=cmd, width=width).pack(side=tk.LEFT, padx=(0, 5))
        ...
        # 統合プレイリストビューワーフレーム
        # ========================================
        integrated_frame = ttk.LabelFrame(
            self.scrollable_frame,
            text="統合プレイリストビューワー",
            padding="15",
            style="Section.TLabelframe"
        )
        integrated_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 20))

        # 統合プレイリスト操作ボタン
        int_btn_frame = ttk.Frame(integrated_frame)
        int_btn_frame.pack(fill=tk.X, pady=(0, 10))

        for text, cmd, width in [("更新", self._refresh_integrated_playlists, 8),
                                  ("新規作成", self._create_new_integrated_playlist, 10),
                                  ("JSON出力", self._export_integrated_json, 10),
                                  ("HTML出力", self._export_integrated_html, 10),
                                  ("削除", self._delete_integrated_playlist, 8)]:
            ttk.Button(int_btn_frame, text=text, command=cmd, width=width).pack(side=tk.LEFT, padx=(0, 5))

        # 統合プレイリストリスト（Treeview）
        int_tree_frame = ttk.Frame(integrated_frame)
        ...
        self.integrated_tree.heading("date", text="作成日時")
        self.integrated_tree.heading("title", text="タイトル")
        self.integrated_tree.heading("videos", text="合計")
        self.integrated_tree.heading("youtube", text="YouTube")
        self.integrated_tree.heading("niconico", text="ニコニコ")
    ...
    def _update_selected_keywords_display(self, *args):
        ...
        if selected:
            self.selected_keywords_var.set(f"選択中: {', '.join(selected)}")
        else:
            self.selected_keywords_var.set("選択中: なし")
    ...
    def _update_selected_regions_display(self, *args):
        ...
        try:
            ...
            if selected:
                if len(selected) <= 5:
                    self.selected_regions_var.set(f"選択中: {', '.join(selected)}")
                else:
                    self.selected_regions_var.set(f"選択中: {', '.join(selected[:5])}... 他{len(selected) - 5}地域")
                self.country_var.set(selected[0])
            else:
                self.selected_regions_var.set("選択中: なし")
    ...
    def _cancel(self):
        """実行をキャンセル"""
        self.is_running = False
        self._update_progress("キャンセルされました")
